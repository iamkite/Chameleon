...
public class NY_StrokeR : MonoBehaviour
{
    ...
    public static NY_StrokeR Instance;
    private void Awake()
    {
        if (Instance == null)
        {
    ...
    void Update()
    {
        ...
        if(GameManager.Instance.view == GameManager.View.Wireframe)
        {
            ...
            //도형 잇기
            else if (curCube != null && OVRInput.Get(OVRInput.Button.PrimaryIndexTrigger, OVRInput.Controller.RTouch) && Vector3.Distance(emptyObject.transform.position, hand.transform.position) > createDistance)
            {
                audios[1].Play();
    ...
    public void set()
    {
        if (output.transform.GetChild(output.transform.childCount - 2).GetComponent<MeshRenderer>().material.name.Contains("stroke"))
        {
            Mesh tempmesh = output.transform.GetChild(output.transform.childCount - 2).GetComponent<MeshFilter>().mesh;
            Color[] tempcolor = tempmesh.colors;
            Vector3[] tempvertices = tempmesh.vertices;
            int[] temptriangles = tempmesh.triangles;
            for (int i = 0; i < tempcolor.Length; i++)
            ...
            for (int i = 0; i < tempcolor.Length; i++)
            {
                tempcolor[i] = (output.transform.GetChild(output.transform.childCount - 2).GetComponent<MeshRenderer>().material.GetTexture("_MainTex") as Texture2D).GetPixel(0,0);
            }

            output.transform.GetChild(output.transform.childCount - 2).GetComponent<MeshRenderer>().material = NY_StrokeR.Instance.material[1];
            tempmesh.Clear();
            tempmesh.vertices = tempvertices;
            tempmesh.triangles = temptriangles;
            tempmesh.colors = tempcolor;
    ...
    public void setlast()
    {
        if (output.transform.GetChild(output.transform.childCount - 1).GetComponent<MeshRenderer>().material.name.Contains("stroke"))
        {
            Mesh tempmesh = output.transform.GetChild(output.transform.childCount - 1).GetComponent<MeshFilter>().mesh;
            Color[] tempcolor = tempmesh.colors;
            Vector3[] tempvertices = tempmesh.vertices;
            int[] temptriangles = tempmesh.triangles;
            for (int i = 0; i < tempcolor.Length; i++)
            ...
            // Debug.Log((output.transform.GetChild(output.transform.childCount - 1).GetComponent<MeshRenderer>()));
            // Debug.Log((output.transform.GetChild(output.transform.childCount - 1).GetComponent<MeshRenderer>().material));
            // Debug.Log((output.transform.GetChild(output.transform.childCount - 1).GetComponent<MeshRenderer>().material.GetTexture("_MainTex") as Texture2D));
            // Debug.Log((output.transform.GetChild(output.transform.childCount - 1).GetComponent<MeshRenderer>().material.GetTexture("_MainTex") as Texture2D).GetPixel(0, 0));
            // Debug.Log((output.transform.GetChild(output.transform.childCount - 1).GetComponent<MeshRenderer>().material.GetTexture("_MainTex") as Texture2D).GetPixel(0, 0));

            for (int i = 0; i < tempcolor.Length; i++)
            {

                tempcolor[i] = (output.transform.GetChild(output.transform.childCount - 1).GetComponent<MeshRenderer>().material.GetTexture("_MainTex") as Texture2D).GetPixel(0, 0);
            }

            output.transform.GetChild(output.transform.childCount - 1).GetComponent<MeshRenderer>().material = NY_StrokeR.Instance.material[1];
            tempmesh.Clear();
            tempmesh.vertices = tempvertices;
            tempmesh.triangles = temptriangles;
            tempmesh.colors = tempcolor;
...
public struct FollowingVertex
{
    //면이 만들어지고 나서 바로 붙는것
    public List<int> frontVertex;
    //다음 면이 만들어지고 나서 붙는것
    public List<int> backVertex;

    public FollowingVertex(Vector3 originalVertex)
    {
        frontVertex = new List<int>();
        backVertex = new List<int>();
    }
