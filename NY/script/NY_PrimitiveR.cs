...
public class NY_PrimitiveR : MonoBehaviour
{
    ...
    public static NY_PrimitiveR Instance;
    private void Awake()
    {
        if (Instance == null)
        {
    ...
    void Update()
    {
        ...
        //트리거를 누르면 거기에 고정
        else if (curCube != null && OVRInput.GetDown(OVRInput.Button.PrimaryIndexTrigger, OVRInput.Controller.RTouch))
        {
            //tempSize = size;
            audios[0].Play();
        ...
        //누르고있는 동안 사이즈 조절
        else if (curCube.layer == 9 && !sizeWaiting && curCube != null && OVRInput.Get(OVRInput.Button.PrimaryIndexTrigger, OVRInput.Controller.RTouch))
        {
        ...
        //최종 생성
        else if (curCube.layer == 9 && OVRInput.GetUp(OVRInput.Button.PrimaryIndexTrigger, OVRInput.Controller.RTouch))
        {
    ...
    IEnumerator DrawingAnimation()
    {
        ...
        //print("draw temp local scale : " + tX);
